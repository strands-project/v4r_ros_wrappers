 ...
 *      Author: Ekaterina Potapova (with adaptions from Markus Suchi for v4r-ros-wrapper library)
 *      
 */
...
{
  ros::init (argc, argv, "object_gestalt_segmentation_server");
  ...
  segmenter_.reset(new v4r::Segmenter);
  segmenter_->setModelFilename(model_filename_);
  segmenter_->setScaling(scaling_filename_);
    
  Segment_ = n_->advertiseService ("/object_gestalt_segmentation", &SegmenterComplete::segment, this);
